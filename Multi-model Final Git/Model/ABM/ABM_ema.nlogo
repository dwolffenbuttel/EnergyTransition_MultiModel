...
to go
  ...
  save-municipality-output

  ;; DEBUGGING WHOO
  ask neighborhoods[
    let a last current-elabels
    let b item 1 reverse current-elabels
    let sum-systems sum-investment-hp + cityheating
    if sum-systems > 1 [
      show sum-systems
    ]
    if sum-investment-hp > 1 [show "sum hp is too big: " print sum-investment-hp]
  ]
...
to update-system-colors
  ...
  [
    ...
    [
      ...
      let systems (list cityheating sum-investment-hp (1 - cityheating - sum-investment-hp))
      set color (ifelse-value
        ...
        max systems = sum-investment-hp [blue]
        [grey] )
    ]
  ]
end
...
to save-municipality-output
  if ema-controls = True
  [
    file-open "municipality-model-outcome.csv"
    file-print csv:to-row [csv:to-row (sentence (list ticks name (available-construction-capacity / construction-capacity)))] of municipalities
    file-close
  ]
end
...
to save-neighbourhood-output
  ;if True = True
  if ema-controls = True and mun-save-list != 0
  [
    file-open "neighbourhood-model-outcome.csv"
    file-print csv:to-row [csv:to-row (sentence (list ticks my-municipality name) temp-suitability (list best_primary_source_option cityheating net-temp  sum-investment-hp (1 - cityheating - sum-investment-hp) current-average-spending-owner) [(sentence (list task most-suitable-contract))] of my-contracts)] of mun-save-list
    file-close
  ]
end
...
to update-HP-costs
  let index-system first SD_systemcost-reduction
  let system-cost-reduction-this-tick item position 2020 time-index SD_systemcost-reduction

  if end? = False
  [
    set system-cost-reduction-this-tick item position ticks time-index SD_systemcost-reduction
  ]
...
to invest-hp
  ...
  let investment-hp-potential-homeowners  map [x -> x * (1 - cityheating) * item 0 property-distribution] (map * (map * item 0 BEP-exceedance-HP cost-undershoot-HP) (list a b a b))
  let investment-hp-potential-corp map [x -> x * (1 - cityheating) * item 1 property-distribution] (map * item 1 BEP-exceedance-HP (list a b a b))
  let investment-hp-potential-rent map [x -> x * (1 - cityheating) * item 2 property-distribution] (map * item 2 BEP-exceedance-HP (list a b a b))
  ...
  ;if sum-investment-hp >= 1 [set investment-hp-potential (list (list 0 0 0 0) (list 0 0 0 0) (list 0 0 0 0))]
  ...
  foreach (list 0 1 2) [ z ->
    ...
    set remaining-list map [x -> min(list x ((1 - sum-investment-hp - cityheating) / 4 ))] remaining-list
  ...
  set sum-investment-hp sum-investment-hp + sum (map - investment-hp-total investment-hp-total_t-1)
   ;set sum-investment-hp sum-investment-hp + sum (map * (map - investment-hp-total investment-hp-total_t-1) (list a b a b))
  ;if sum-investment-hp > 1 [ set sum-investment-hp 1]
...
to calculate-ROI
  ...
  set adjusted-private-ROI max (list min-ROI (base-private-ROI * neighbor-effect / [current-willingness-factor] of municipality-agent))
...
to calculate-current-elabels
  ...
  let comp-factor 1 + abs(sum (map [[x y] -> min(list x y)] current-elabels (list 0 0 0 0 0 0 0)))
  if comp-factor > 1
  [
    set current-elabels map [x -> x / comp-factor] current-elabels
    set current-elabels (map [[x y] -> max(list x y)] current-elabels (list 0 0 0 0 0 0 0))
  ]
...
to calculate-HG-investment-cost
  ...
  let b item 1 reverse current-elabels;
  ;; NEW HEATING GRIDS
  let total_net_connections total_houses * max(list 0 (1 - cityheating - sum-investment-hp))
...
to setup-new-neighborhood
  ...
  (ifelse system = "Gas" [set investment-hp-total [ 0 0 0 0 ] ]
    ...
    system = "Heat pump" [set investment-hp-total [ 0.5 0 0.5 0 ] set sum-investment-hp 1] )
...
to aggregate-neighborhood-data
  ...
  let gas sum [total_houses *  (1 - cityheating - sum-investment-hp)] of my-neighborhoods / total_houses
...
to init-agents
  ...
  set mun-save-list neighborhoods with [my-municipality = "Amsterdam"]
  set end? false
  reset-ticks
  tick-advance 2020
  update-plots
...
to reset-save-files
  ...
  [
    ...
    csv:to-file "municipality-model-outcome.csv" []
...
to import_constants
  ...
  let value-index position heating-grid-cost-scenario item 0 ABM-constants
